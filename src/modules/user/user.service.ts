...
import { RegisterData, UpdatePasswordData, VerifyEmailData } from "./user.type";
import { createToken } from "../../utils/tokenisation";

// Register Function
async function register({ res, data }: { res: Response; data: RegisterData }) {
  let user;
  try {
    const { fullName, phone, gender, address, email, password } = data;
    user = await new User({
      fullName,
      phone,
      address,
